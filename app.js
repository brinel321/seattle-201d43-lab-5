...
function sumAndMultiply(a, b, c) { //eslint-disable-line
  var totalSum = sum(sum(a, b)[0], sum(0, c)[0])[0];
  var totalMultiply = multiply(multiply(a, b)[0], multiply(1, c)[0])[0];
...
function sumArray(sumArr) { //eslint-disable-line
  var totalSum = sum(sum(sumArr[0], sumArr[1])[0], sum(0, sumArr[2])[0])[0];
...
function multiplyArray(multArr) { //eslint-disable-line
  var totalMult = multiply(multiply(multArr[0], multArr[1])[0], multiply(1, multArr[2])[0])[0];
...
function multiplyAnyArray(dynamicArray) { //eslint-disable-line
  ...
  for(var i = 0; i < dynamicArray.length; i++){
    totalMult = multiply(dynamicArray[i], totalMult)[0];
    if(i === dynamicArray.length - 1){
      response = response + dynamicArray[i];
    }else{
      response = response + dynamicArray[i] + ',';
    }
  }

return[totalMult, 'The numbers ' + response + ' have a product of ' + totalMult + '.'];
}
